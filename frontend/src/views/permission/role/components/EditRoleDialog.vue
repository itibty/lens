 ...
 * @LastEditTime: 2024-11-25 14:07:30
 * @LastEditors: Chuang
 * @Description: 编辑角色弹窗
-->
...
import { editRole } from '@/apis/admin/role'
import CustomDialog from '@/components/CustomDialog.vue'
import { showToast } from '@/utils/index'
import { isBlank } from '@/utils/validate'

export interface EditRoleDialogInstance {
  showDialog: (row: ADMIN.RoleInfo) => void
}
...
function showDialog(row: ADMIN.RoleInfo) {
  states.form = pick(row, Object.keys(states.form)) as ADMIN.SaveRoleRequest
  states.form.roleCode = row.roleCode || ''
  dialog.visible = true
}
...
function doSubmit() {
  formRef.value?.validate(async (valid) => {
    if (valid) {
      ...
      editRole(body as ADMIN.SaveRoleRequest)
        .then((res) => {
          const { msg } = res
