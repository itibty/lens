...
const props = defineProps<{ query?: VisQueryConfig }>()
...
const issue = computed(() => axisBoundsIssue(axis.value))
...
function reset() {
  const axes = { ...visual.value.chart?.axes }
  delete axes[role.value]
  visual.value.chart = { ...visual.value.chart, axes }
}
</script>

<template>
  <div class="vis-style-form__row">
    <StyleFormLabel>坐标轴</StyleFormLabel>
    <el-select v-model="role" size="small" class="vis-style-form__control">
  ...
  <div v-if="axis.showTitle ?? defaultTitleVisible" class="vis-style-form__row">
    <StyleFormLabel>标题</StyleFormLabel>
    <el-input :model-value="axis.title ?? ''" size="small" class="vis-style-form__control" placeholder="默认标题" clearable :maxlength="100" @update:model-value="patch('title', $event)" />
  </div>
  ...
  <div class="vis-style-form__row">
    <el-button text size="small" :disabled="!Object.keys(axis).length" @click="reset">
      恢复默认
    </el-button>
  </div>
</template>
